...
class ModerationSystem:
    ...
    def __init__(self):
        # Load the schema
        with open("examples/content_moderation/schema.amino") as f:
    ...
    def _contains_banned_words(self, text: str) -> bool:
        ...
        banned_words = [
            "badword1",
            "badword2",
            "offensive_term",  # Placeholder terms
        ]
    ...
    def _is_potential_harassment(self, text: str, target_user: str) -> bool:
        ...
        harassment_indicators = ["you should", "you are such a", "people like you", "why don't you", "shut up"]
    ...
    def moderate_content(
        self, user_data: dict[str, Any], content_data: dict[str, Any], context_data: dict[str, Any]
    ) -> dict[str, Any]:
        """Evaluate content against moderation rules."""

        # Combine all data for rule evaluation
        moderation_data = {"user": user_data, "content": content_data, "context": context_data}

        # Compile rules for evaluation (first match wins - highest priority)
        compiled_rules = self.schema.compile(
            self.moderation_rules, match={"option": "first", "key": "ordering", "ordering": "asc"}
        )
...
def main():
    ...
    # Sample user data
    users = [
        {"id": "user_001", "account_age_days": 2, "reputation_score": 0.8, "verified": False, "previous_violations": 0},
        {
            "id": "user_002",
    ...
    for i, user in enumerate(users):
        print(f"\n👤 User {i + 1}: {user['account_age_days']} days old, {user['previous_violations']} violations")
        ...
        for j, content in enumerate(content_samples):
            ...
            action_emoji = {"approve": "✅", "flag": "🏴", "quarantine": "⚠️", "remove": "❌"}.get(
                decision["action"], "❓"
            )
