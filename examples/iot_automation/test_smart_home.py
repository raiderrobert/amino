...
class TestSmartHomeSystem:
    ...
    def home_context_home(self):
        ...
        return {"occupancy_status": "home", "security_mode": "disarmed", "time_of_day": "afternoon", "season": "summer"}

    @pytest.fixture
    def home_context_away(self):
        """Home context when family is away."""
        return {"occupancy_status": "away", "security_mode": "armed", "time_of_day": "evening", "season": "summer"}
    ...
    def test_climate_control_when_home_hot(
        self, system, sensor_data_hot, device_state_normal, home_context_home, user_preferences_auto
    ):
        """Test climate control activation when home and too hot."""
        actions = system.process_iot_data(
            sensor_data_hot, [device_state_normal], home_context_home, user_preferences_auto
        )
    ...
    def test_climate_control_comfortable_temp(
        self, system, sensor_data_normal, device_state_normal, home_context_home, user_preferences_auto
    ):
        """Test no climate action when temperature is comfortable."""
        actions = system.process_iot_data(
            sensor_data_normal, [device_state_normal], home_context_home, user_preferences_auto
        )
    ...
    def test_security_motion_detection(
        self, system, sensor_data_motion, device_state_normal, home_context_away, user_preferences_auto
    ):
        """Test security alert when motion detected while armed."""
        actions = system.process_iot_data(
            sensor_data_motion, [device_state_normal], home_context_away, user_preferences_auto
        )
    ...
    def test_no_security_alert_when_disarmed(
        self, system, sensor_data_motion, device_state_normal, home_context_home, user_preferences_auto
    ):
        """Test no security alert when motion detected but system disarmed."""
        actions = system.process_iot_data(
            sensor_data_motion, [device_state_normal], home_context_home, user_preferences_auto
        )
    ...
    def test_auto_lights_evening_dark(self, system, device_state_normal, home_context_away, user_preferences_auto):
        ...
        actions = system.process_iot_data(
            dark_sensor_data, [device_state_normal], home_context_away, user_preferences_auto
        )
    ...
    def test_no_auto_lights_when_disabled(
        self, system, device_state_normal, home_context_away, user_preferences_manual
    ):
        ...
        actions = system.process_iot_data(
            dark_sensor_data, [device_state_normal], home_context_away, user_preferences_manual
        )
    ...
    def test_energy_usage_alert(
        self, system, sensor_data_normal, device_state_high_power, home_context_home, user_preferences_auto
    ):
        ...
        with patch.object(system, "_get_average_consumption", return_value=180.0):
            actions = system.process_iot_data(
                sensor_data_normal, [device_state_high_power], home_context_home, user_preferences_auto
            )
    ...
    def test_air_quality_response(
        self, system, sensor_data_poor_air, device_state_normal, home_context_home, user_preferences_auto
    ):
        """Test air quality response for poor air."""
        actions = system.process_iot_data(
            sensor_data_poor_air, [device_state_normal], home_context_home, user_preferences_auto
        )
    ...
    def test_sleep_mode_activation(
        self, system, sensor_data_normal, device_state_normal, home_context_sleep, user_preferences_auto
    ):
        """Test sleep mode activation at night."""
        actions = system.process_iot_data(
            sensor_data_normal, [device_state_normal], home_context_sleep, user_preferences_auto
        )
    ...
    def test_rule_priority_order(self, system, device_state_normal, home_context_away, user_preferences_auto):
        ...
        actions = system.process_iot_data(
            multi_rule_sensor_data, [device_state_normal], home_context_away, user_preferences_auto
        )
    ...
    def test_edge_cases(self, system, user_preferences_auto):
        ...
        actions = system.process_iot_data(
            extreme_sensor_data, [extreme_device_state], home_context, user_preferences_auto
        )
        ...
        actions = system.process_iot_data(
            empty_sensor_data, [extreme_device_state], home_context, user_preferences_auto
        )
        assert isinstance(actions, list)

    def test_multiple_simultaneous_triggers(
        self, system, device_state_normal, home_context_home, user_preferences_auto
    ):
        ...
        actions = system.process_iot_data(
            complex_sensor_data, [device_state_normal], home_context_home, user_preferences_auto
        )
    ...
    def test_notification_preferences(
        self, system, sensor_data_motion, device_state_normal, home_context_away, user_preferences_manual
    ):
        """Test that notification preferences are respected."""
        # User has disabled security notifications
        actions = system.process_iot_data(
            sensor_data_motion, [device_state_normal], home_context_away, user_preferences_manual
        )
    ...
    def test_time_based_automation(self, system, sensor_data_normal, device_state_normal, user_preferences_auto):
        ...
        morning_actions = system.process_iot_data(
            sensor_data_normal, [device_state_normal], morning_context, user_preferences_auto
        )

        evening_actions = system.process_iot_data(
            sensor_data_normal, [device_state_normal], evening_context, user_preferences_auto
        )

        night_actions = system.process_iot_data(
            sensor_data_normal, [device_state_normal], night_context, user_preferences_auto
        )
