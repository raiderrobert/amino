...
class ExampleValidator:
    ...
    def validate_example_execution(self, example_name: str) -> bool:
        ...
        try:
            ...
            self.results["failed"].append(f"{example_name}: Execution error - {e!s}")
            return False

    def _find_main_file(self, example_path: Path) -> Path | None:
        """Find the main Python file for an example."""
        # Common main file patterns
        patterns = [f"{example_path.name}.py", "main.py", f"{example_path.name.replace('_', '')}.py"]
    ...
    def run_example_tests(self, example_name: str) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["uv", "run", "python3", "-m", "pytest", relative_path, "-v", "--tb=short", "-q"],
                capture_output=True,
                text=True,
                cwd=str(self.examples_dir.parent),
            )
            ...
            self.results["failed"].append(f"{example_name}: Test execution error - {e!s}")
            return False
    ...
    def validate_schema_files(self) -> bool:
        ...
        for example_dir in self.examples_dir.iterdir():
            ...
            try:
                ...
                schema_issues.append(f"{example_dir.name}: Schema file read error - {e!s}")
    ...
    def validate_all_examples(self) -> dict:
        ...
        example_dirs = [d for d in self.examples_dir.iterdir() if d.is_dir() and not d.name.startswith(".")]
